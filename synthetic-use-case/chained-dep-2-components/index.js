...
const program = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const contentManager = new resources_1.RemoteConnection(`server`, { port: 19952, host: serverHost });
	...
	let depWish = new resources_1.Wish(contentManager, `serverProvide${reconfiguration_name}`);
	depWish.offer.apply(res =>{
		console.log(`RESOURCES OFFER: ${res}`)
	})
    const depInstallRessource = new depInstall_1.DepInstallResource(`dep${depNum}Install${reconfiguration_name}`, { reconfState: reconfiguration_name, time: installTime, depsOffers: [depWish.offer] });
    // new resources_1.Offer(contentManager, `dep${depNum}${reconfiguration_name}`, depInstallRessource);
    // logger.info(`dep${depNum} deployTime: ${deployTime}`);
    // const depRunningRessource = new depInstall_1.DepInstallResource(`dep${depNum}Running${reconfiguration_name}`, 
	// 	{ reconfState: reconfiguration_name, time: deployTime }, { dependsOn: depInstallRessource }
	// );
	...
	depInstallRessource.id.apply(resultId => {
    ...
    return {
        chainedDepInstallId: depInstallRessource.id
...
core_1.runDeployment(initStack, core_1.operations(hareactive_1.Behavior.of(program)), (offerUpdates) => core_1.nextAction(offerUpdates, core_1.sigquit(), core_1.sigint()), { deploymentName: `dep${depNum}`, resourcesPort: 21000-1, deploymentPort: 21000 });
